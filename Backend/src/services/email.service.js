...
const hasBrevoApi = Boolean(config.BREVO_API_KEY);
const hasSmtpConfig = Boolean(
    config.BREVO_SMTP_HOST && config.BREVO_SMTP_USER && config.BREVO_SMTP_PASS,
);

const fromAddress = config.EMAIL_FROM || config.BREVO_SMTP_USER || config.GOOGLE_USER;

let smtpTransporter = null;

if (hasBrevoApi) {
    isEmailTransportReady = true;
    console.log('Email provider selected: Brevo API');
    ...
    isEmailTransportReady = false;
    lastEmailError = 'No email provider configured';
    ...
    try {
        if (hasBrevoApi) {
            await axios.post(
                ...
                {
                    sender: { email: fromAddress, name: 'Nova Auth' },
                    to: [{ email: to }],
                    subject,
                    textContent: text,
                    htmlContent: html,
